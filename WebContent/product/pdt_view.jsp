...
<% if (!isLogin) {	// 로그인 전이면(로그인 후 다시 돌아옴) %>
	...
	// request.getRequestURI()의 리턴타입은 StringBuffer이므로 replace()메소드를 사용하기 위해 String으로 형변환 후 작업함

<% } else {	// 로그인 상태면 해당 위치로 전송시킴 %>
	var lnk = "";
	if (chk == 'c')	lnk = "cart_proc.ord";	// 장바구니에 담기일 경우 이동할 경로
...
<% } %>
	...
	<td width="*" valign="top">
		...
		<input type="hidden" name="bcate" value="<%=pageInfo.getBcata() %>" />
		<input type="hidden" name="scate" value="<%=pageInfo.getScata() %>" />
