...
public class CartListAct implements Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (request.getParameter("psize") == null)	pdtPageInfo.setPsize(8);
		else	pdtPageInfo.setPsize(Integer.parseInt(request.getParameter("psize")));

		pdtPageInfo.setKeyword(request.getParameter("keyword"));	pdtPageInfo.setBcata(request.getParameter("bcata"));
		pdtPageInfo.setScata(request.getParameter("scata"));		pdtPageInfo.setBrand(request.getParameter("brand"));
		pdtPageInfo.setSprice(request.getParameter("sprice"));		pdtPageInfo.setEprice(request.getParameter("eprice"));
